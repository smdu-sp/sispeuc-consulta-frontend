...
export default function UsuarioDetalhes() {
	...
	useEffect(() => {
		FetchValidaUsuario().then((response: IUsuario) => {
			setUsuario(response);
	...
	return (
		<Content
			breadcrumbs={[{ label: 'Meu Perfil', href: '/eu' }]}
			titulo={usuario ? usuario.nome : 'Meu Perfil'}
			tags={
				usuario ? (
					<div style={{ display: 'flex', gap: '0.2rem' }}>
						<Chip
							color={permissoes[usuario?.permissao].color}
							size='lg'>
							{permissoes[usuario?.permissao].label}
						</Chip>
					</div>
				) : null
			}
